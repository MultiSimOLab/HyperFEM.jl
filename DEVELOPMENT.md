...
The registrator will open a PR in the Julia registry. Once the package is published, the tag bot will create a release and generate the changelog.

#### ⚠️ Breaking changes (increase on first non-zero digit `0.0.1 -> 0.0.2`)

If so, you will need to tell the registrator which are the breaking changes:
```
@JuliaRegistrator register
...
Documentation is generated by `docs/make.jl` and triggered by `workflows/docs.yml`.
